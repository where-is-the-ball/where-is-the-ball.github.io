...
const cameraPresets = {
  ...
  customView: {
    ...
    quaternion: [
      -0.11315931687990666, -0.7436465544931189, -0.1302802493805461,
      0.6459193661867775,
    ],
  ...
  closeup_net: {
    ...
    quaternion: [
      -0.059453852317, 0.218891007049, 0.013363439154, 0.973844641086,
    ],
    fov: 22.277465448544,
  },
  closeup_net_tid14: {
    ...
    quaternion: [
      -0.023340865563, 0.965885327077, 0.093842793181, 0.240237942672,
    ],
    fov: 22.277465448544,
  },
  closeup_net_tid11: {
    ...
    quaternion: [
      -0.053961108143, -0.275038858689, -0.015463126611, 0.959793059328,
    ],
    fov: 22.277465448544,
  },
  ...
  default: {
    ...
    quaternion: [
      -0.127621041211, 0.000960714622, 0.000123618285, 0.991822530288,
    ],
    fov: 22.277465448544,
  },
};
...
function setupGUI() {
  ...
  var props = {
    courtview: function () {
      camera.position.set(
        helper_camera.position.x,
        helper_camera.position.y,
        helper_camera.position.z
      );
...
readData(function () {
  $(document).ready(function () {
    ...
    $(document).keydown(function (e) {
      ...
      config.traj_id = Math.min(
        Object.keys(data).length - 1,
        Math.max(0, config.traj_id)
      );
    ...
    let secondViewTriggered = false;
    let resetDefaultTrigger = false;
    ...
    renderer.setAnimationLoop(function (time) {
      ...
      if (config.animate) {
        ...
        if (id0 >= 15 && id0 <= 52 && config.traj_id === 10) {
        ...
        // 2) at frame 53: reset to default (only once)
        if (
          config.traj_id === 10 &&
          slowMoTriggered && // only after first zoom has started
          id0 === 53 && // your chosen “reset” frame
          !resetDefaultTrigger
        ) {
          resetDefaultTrigger = true;
          transitionToPreset("default", 1400);
        }
    if (config.traj_id === 10 && id0 >= 100 && id0 <= 133) {
      trajSpeed = 0.25;  // same slow-mo factor
      if (!secondViewTriggered) {
        secondViewTriggered = true;
        ...
        $("#imgclip").attr(
          "src",
          `./clips/${cd.g_name}/${cd.c_name}/${String(
            id0 + cd.f_start
          ).padStart(4, "0")}.jpg`
        );
...
function transitionToPreset(name, duration = 1000) {
  ...
  const targetPos = Array.isArray(raw.position)
    ? new THREE.Vector3(...raw.position)
    : raw.position.clone();

  const targetQuat = Array.isArray(raw.quaternion)
    ? new THREE.Quaternion(...raw.quaternion)
    : raw.quaternion.clone();
  ...
  new TWEEN.Tween({ t: 0 })
    ...
    .easing(TWEEN.Easing.Quadratic.InOut)
    .onUpdate((o) =>
      camera.quaternion.slerpQuaternions(startQ, targetQuat, o.t)
    )
    .start();
