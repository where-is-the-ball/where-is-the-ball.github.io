...
const cameraPresets = {
	...
	closeup_net_tid11: {
		position: [-6.521441715079, 2.65150563926, 8.095461348983],
		quaternion: [-0.053961108143, -0.275038858689, -0.015463126611, 0.959793059328],
		fov: 22.277465448544,
	},
	closeup_net_tid10_ntf: {
		position: [-1.915104067946, 2.125681268891, 6.785769220477],
		quaternion: [-0.070635987128, -0.053243477867, -0.003775732898, 0.996073005973],
		fov: 22.277465448544,
	},
	// : {
	//   position: [-2.171663394064, 2.397405390263, 7.185359973746],
	//   quaternion: [
	//     -0.062300831033, -0.048988774192, -0.003061686562, 0.996849704083,
	//   ],
	//   fov: 22.277465448544,
	// },
	closeup_net_tid10_ftn:
		// position: [3.299869415876, 2.060282133279, 8.935508066156],
		// quaternion: [
		//   -0.058425500815, 0.134811654057, 0.007963115368, 0.989115194292,
		// ],
		// fov: 22.277465448544,
		{
			position: [2.801245474513, 1.890926563386, 5.581815371222],
			quaternion: [-0.075040486046, 0.085575168935, 0.006463740898, 0.99348076779],
			fov: 22.277465448544,
		},
	default: {
		position: [0.063627328512, 8.594541269435, 32.843758843512],
		quaternion: [-0.127621041211, 0.000960714622, 0.000123618285, 0.991822530288],
		fov: 22.277465448544,
	},
};
...
function unlineColoring(point) {
	return new THREE.LineBasicMaterial({
		color: new THREE.Color(43 / 255, 185 / 255, 221 / 255),
	});
}
function sliderOnChange(min, max, key) {
	...
	$("#imgclip").attr("src", "./clips_tracking/" + cd.g_name + "/" + cd.c_name + "/" + (key + cd.f_start).pad(4) + ".jpg");
}
...
// ─── define your orbit helper ───────────────────────────────────────────────
function orbitCamera(duration = 5000) {
	// look‐at point for the orbit:
	const target = controls.target.clone();

	// compute current angle & radius in the XZ plane:
	const offset = new THREE.Vector2(camera.position.x - target.x, camera.position.z - target.z);
	const radius = offset.length();
	const startAngle = Math.atan2(offset.y, offset.x);

	new TWEEN.Tween({ θ: startAngle })
		.to({ θ: startAngle + Math.PI * 2 }, duration)
		.easing(TWEEN.Easing.Linear.None)
		.onUpdate((o) => {
			camera.position.x = target.x + radius * Math.cos(o.θ);
			camera.position.z = target.z + radius * Math.sin(o.θ);
			camera.lookAt(target);
		})
		.start();
}

function hideAll(is_hide) {
	showAllController.setValue(!is_hide);
	showLineController.setValue(!is_hide);
	config.showall = !is_hide;
	config.showline = !is_hide;
}
...
readData(function () {
	$(document).ready(function () {
		...
		// “fired” flags so everything only happens once:
		let firstZoomTriggered = false,
			firstResetTriggered = false,
			secondZoomTriggered = false,
			secondResetTriggered = false;
      cycleComplete = false;
      ...
      orbitDone = false;
		...
		renderer.setAnimationLoop(function (time) {
			...
			if (config.animate) {
				...
				// ——— First slow-mo & zoom (frames 15–52) ———
				if (!cycleComplete && id0 >= 15 && id0 <= 52) {
					trajSpeed = 0.25; // 25% speed
					if (!firstZoomTriggered) {
						firstZoomTriggered = true;
            hideAll(true)
						transitionToPreset("closeup_net_tid10_ntf", 1100);
					}
				}

				// reset back to default right after frame 52
				if (firstZoomTriggered && id0 > 52 && !firstResetTriggered) {
					firstResetTriggered = true;
					transitionToPreset("default", 1400);
          hideAll(false)
				}

				// ——— Second slow-mo & zoom (frames 100–133) ———
				if (!cycleComplete && id0 >= 100 && id0 <= 133) {
					trajSpeed = 0.25; // same slow-mo factor
          hideAll(true)
					if (!secondZoomTriggered) {
						secondZoomTriggered = true;
						transitionToPreset("closeup_net_tid10_ftn", 1200);
					}
				}

				// reset back to default right after frame 133
				if (secondZoomTriggered && id0 > 133 && !secondResetTriggered) {
					secondResetTriggered = true;
          hideAll(false)
					transitionToPreset("default", 1400);
					cycleComplete = true; // ← from now on, no more slow-mo
				}

				if (!orbitTriggered && cycleComplete && id0 < 50) {
					orbitTriggered = true;
					orbitCamera(10000);
          setTimeout(() => {
            hideAll(true)
          ...
          setTimeout(() => {
            hideAll(false)
				...
				$("#imgclip").attr(
					"src",
					`./clips_tracking/${cd.g_name}/${cd.c_name}/${String(id0 + cd.f_start).padStart(4, "0")}.jpg`
				);
