...
function drawFlatLine(scene, a, b, w, stand = 0, color = 0xffffff, castShadow = false) {
	if (Array.isArray(a)) a = new THREE.Vector3(a[0], a[1], a[2]);
	if (Array.isArray(b)) b = new THREE.Vector3(b[0], b[1], b[2]);

	if (stand) var up = new THREE.Vector3(0.5, 0, 0.5);
	else var up = new THREE.Vector3(0, 1, 0);
...
function setupGUI() {
	...
	folder_traj
		.add(config, "showgt")
		.name("Show Ground truth 🔴")
		.listen()
		.onChange(function (value) {
			for (let i = 0; i < gt_traj.children.length; i++) {
				gt_traj.children[i].visible = value && i >= min && i <= max;
			}
		});
	folder_traj
		.add(config, "showun")
	...
	//});
	folder_traj
		.add(config, "ballsize_base", 0.01, 0.1, 0.01)
		.name("Ball radius")
		.listen()
		.onChange(function (value) {
			customAddTrajectory();
	...
	folder_traj
		.add(config, "tballsize_base", 0.01, 0.1, 0.01)
...
function unColoring(point, simple = false) {
	...
	const color = [255 / 255, 136 / 255, 0 / 255];
...
function predColoring(point, simple = false) {
	const int = 0.3;
	const em = 0.7;
...
function gtColoring(point, simple = false) {
	const int = 0.3;
	const em = 0.8;
	const color = [180 / 255, 27 / 255, 33 / 255];
	if (simple) return new THREE.MeshBasicMaterial({ color: 0xca1e26 });
	return new THREE.MeshPhongMaterial({
		color: new THREE.Color(color[0] * int, color[1] * int, color[2] * int),
		emissive: new THREE.Color(color[0] * em, color[1] * em, color[2] * em),
		specular: new THREE.Color(0.1, 0.1, 0.1),
...
function sliderOnChange(min, max, key) {
	...
	if (config.showgt) {
		for (let i = 0; i < gt_traj.children.length; i++) {
			gt_traj.children[i].visible = i >= min && i <= max;
		}
	}
	if (config.showun) {
		for (let i = 0; i < un_traj.children.length; i++) {
...
function analyzeTrajectory(data) {
	function getPrincipleDirection(d) {
		...
		for (let j = 0; j < 3; j++) mean[j] /= d.length;
		for (let i = 0; i < d.length; i++) {
	...
	let svec = getPrincipleDirection(data["pred_refined"]);
	console.log(svec);

	rotating(data["pred_unrefined"], svec);
	rotating(data["pred_refined"], svec);
	...
	fb(data["pred_unrefined"]);
	fb(data["pred_refined"]);
	fb(data["gt"]);

	centering(data["pred_unrefined"]);
	centering(data["pred_refined"]);
	centering(data["gt"]);

	let heights = [0, 0];
	const d = data["pred_refined"];
	for (let i = 0; i < d.length; i++) heights[d[i][0] < 0 ? 0 : 1] += d[i][1];
	if (heights[1] > heights[0]) {
		rotating(data["pred_unrefined"], [-1, 0, 0]);
		rotating(data["pred_refined"], [-1, 0, 0]);
	...
	if (center != null) {
		console.log("In1", center.y);
		helper_camera.position.set(center.x, center.y, center.z);
		helper_camera.lookAt(lookat.x, lookat.y, lookat.z);
	} else {
		helper_camera.position.set(zoom * -6.098631031333716, zoom * 4.544654504527356, zoom * 14.427502035284304);
...
function setupScene(scene, center) {
	...
	helper_camera = new THREE.PerspectiveCamera(fov, 1, 0.1, 1000);
	if (center != null) {
		console.log("In1", center.y);
		helper_camera.position.set(center.x, center.y, center.z);
		helper_camera.lookAt(lookat.x, lookat.y, lookat.z);
	...
	axisline.push(
		drawFlatLine(
			scene,
			[-config.cb_size * 0.5, 0, config.cb_size * 0.5],
			[config.cb_size * 0.5, 0, config.cb_size * 0.5],
			0.03,
			1,
			0xaaaaaa
...
readData(function () {
	$(document).ready(function () {
		...
		for (const [key, value] of Object.entries(data)) {
			rescaling(data[key]["pred_unrefined"]);
			rescaling(data[key]["pred_refined"]);
			rescaling(data[key]["gt"]);
		}
		...
		if (camInit != 0 && camInit != null) {
			...
			if (sp.length == 6) {
				center = new THREE.Vector4(0, 0, 0, 1);
				center.x = parseFloat(sp[0]);
				center.y = parseFloat(sp[1]);
				center.z = parseFloat(sp[2]);
		...
		//for (var i = 0; i < data["0"]["pred_refined"].length; i++) {
		//data["1"]["pred_refined"][i][2] *= -1;
		//}
...
function render() {
	let dim = [0, 0, 0];
	for (let i = 0; i < 3; i++) dim[i] = (bound[i][1] - bound[i][0]) * 1.1;
	...
	const simpleBlue = predColoring(null, true);
	const simpleRed = gtColoring(null, true);
	...
	for (let ii = 0; ii < views.length; ++ii) {
		...
		if (ii == 0) {
			...
			axisline[0].visible = false;
			axisline[1].visible = false;
			cb.visible = true;
		} else if (ii == 1 || ii == 2) {
			// XY plane
			axisline[0].visible = true;
			axisline[1].visible = true;
			cb.visible = false;
		...
		if (ii == 1) {
			for (let i = 0; i < traj.children.length; i++) traj.children[i].material = simpleBlue;
			for (let i = 0; i < gt_traj.children.length; i++) gt_traj.children[i].material = simpleRed;
		}
		renderer.render(scene, c);
	}
	for (let i = 0; i < traj.children.length; i++) traj.children[i].material = oldBlue;
	for (let i = 0; i < gt_traj.children.length; i++) gt_traj.children[i].material = oldRed;
